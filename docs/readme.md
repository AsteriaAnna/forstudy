...
# 指定运行ID和恢复
python main.py --stage all --run-id run_001_20250524_1530 --resume
```
...
| `wear_label` | dict | - | {flute_1, flute_2, flute_3} |
| `status` | str | - | "LOADED" |
...
| `check_channel_correlation(signal)` | 信号数组 | bool | 检测通道相关性 |
...
| `time_shift(features, shift_range)` | 特征, 偏移范围 | 增强特征 | 时间偏移 |
| `sliding_window(signal, window_size, step)` | 信号, 窗口, 步长 | 窗口列表 | 滑动窗口切片 |
| `bin_wear_category(wear_label)` | 磨损标签 | category字符串 | 磨损等级分类 |
...
```python
{
    ...
    "labels_list": List[dict],           # 每个元素 {flute_1, flute_2, flute_3}
    "tool_ids": List[str],
